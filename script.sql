...
CREATE TABLE Countries_2020(id_country serial primary key, country VARCHAR( 100 ), population integer, 
						 yearly_change numeric, net_change integer, density INTEGER ,
					     land_area INTEGER, migrants numeric, fert_rate numeric, med_age integer,
					     urban_pop integer, world_share numeric);
						 
--INSERTION DES DONNEES DANS LA TABLE
-- INSERT QUERY 
INSERT INTO countries_2020( Country,population,yearly_change,net_change,density,land_area,migrants,fert_rate,med_age,urban_pop,world_share )
VALUES
(
...
--CREATION DE LA FONCTION get_country_proprieties()


CREATE OR REPLACE FUNCTION public.get_country_proprieties(
	country_name character varying)
    RETURNS  setof countries_2020
AS $BODY$
begin
return query 
...
--CREATION DE LA PROCEDURE insert_country()


create or replace procedure insert_country(new_country VARCHAR)
language plpgsql    
as $$
declare 
v_pop integer; 
v_yearly_change numeric;
...
v_med_age integer;
v_urban_pop integer;
v_world_share numeric;

begin
select  floor(random()*33097732 ) into v_pop;
select floor (random()*3) into v_yearly_change;
select floor (random()*346087) into v_net_change;
select floor (random()*475.770) into v_density;
select floor (random()*553591.84) into v_land_area;
select floor (random()*5000) into v_migrants;
select floor (random()*4) into v_fert_rate;
select floor (random()*30.6) into v_med_age;
select floor (random()*59.23) into v_urban_pop;
select floor (random()*16) into v_world_share;

INSERT INTO countries_2020( Country,population,yearly_change,net_change,density,land_area,migrants,fert_rate,med_age,urban_pop,world_share )
VALUES (new_country, v_pop,v_yearly_change ,v_net_change, v_density,v_land_area,v_migrants,v_fert_rate,v_med_age,v_urban_pop,v_world_share );
end; $$;

--CREATION D'UN TRIGGER 

-- AJOUTER UNE COLONNE DATE A LA TABLE countries_2020

ALTER TABLE countries_2020 ADD insertion_date DATE;

--CREATION DE LA FONCTION update_insertion_date() 

CREATE function update_insertion_date()
  RETURNS TRIGGER 
  LANGUAGE PLPGSQL  
  AS $$
  DECLARE
...
UPDATE countries_2020 set insertion_date = v_date
WHERE id_country = NEW.id_country;
RETURN NEW;
end;
$$;
-- TRIGGER insertion_date

CREATE  TRIGGER insertion_date AFTER insert
ON countries_2020
FOR EACH  ROW 
       EXECUTE PROCEDURE  update_insertion_date();
	   


-- CREATION DE LA FONCTION QUI VA RETOURNER LES PAYS REGROUPES PAR TRANCHE get_country_tranche()

create or replace function get_country_tranche()
returns table(country VARCHAR, density int, tranche text)
language plpgsql
as $$
begin
return query 
   select c.country, c.density,
   case
   WHEN 0 < c.density and c.density<300 then 'Tranche1'
   WHEN 300 < c.density and c.density <500 then 'Tranche2'
   WHEN 500 < c.density and c.density <1000 then 'Tranche3'
   WHEN c.density >1000 then 'Tranche4'
   END as tranche
   from countries_2020 c;
 end;$$ 
